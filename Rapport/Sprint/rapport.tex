...
notamment grâce à l'ajout de tests unitaires de d'intégration qui sont, pour l'instant, manquants.
Dans un même temps, nous ajouterons une fonctionnalité à l'interface graphique
afin de permettre une configuration plus simple de celle-ci et de ses paramètres.

Cette librarie, étant codé en Rust, nous travaillerons donc avec ce langage. 
Pour l'IDE, nous utiliserons Visual Studio Code avec l'extension Rust Analyzer.
Pour ce qui est de l'environnement de développement, nous utiliserons GitHub pour le
suivi et un sous-environnement Ubuntu pour le développement grâce à WSL.
...
\\On va surtout travailler sur le back end pour pouvoir changer de contact plan et d'algorithme à la volée depuis l'interface.
...
\begin{itemize}
    \item \textbf{Tests et Pipelines :} Yassine Jouini et Bastien Dague
    \item \textbf{Modifications de l'interface graphique :} Samy Al-Faraj et Vincent Brochard
...
Le dépôt centralise tous les sous-projets (A-SABR, interface graphique, Hardy) sous forme de sous-modules
qui redirigeront chacun vers des dépôts fork des travaux de Monseur De Jonckère.
Pour éviter tout problème de conflits, notre dépôt suivra la structure GitFlow.
Chaque membre du groupe travaillera sur sa propre branche et des merge réguliers sur la branche
develop seront faits. L'envoi sur la branche main ne se fera qu'après s'être assuré
de la stabilité du projet. Après cela, nous aurons la possibilité de faire
des Pull Requests avec l'accord de notre encadrant pour contribuer aux dépôts originaux.

Enfin, pour la communication interne du groupe, nous utiliserons Discord 
pour nos discusssions. Pour le moment, la communication avec notre encadrant se fera par mail
mais nous envisagerons de communiquer par messagerie instantanée quand le projet sera plus avancé. 
...
Pour la seconde phase, nos priorités seront d'être le plus exhaustif possible dans nos tests
afin de permettre une meilleure testabilité présente et future et de permettre une utilisation
plus confortable de l'interface graphique en se passant de lignes de commandes complexes.
