...
Elle peut manquer le chemin le plus optimal car s'arrête dès qu'un chemin optimal est trouvé par rapport à un autre. \\

Contact explore lui le \gls{mg} par contact plutôt que par noeud, il va utiliser une "Work Area", permettant
d'ignorer des noeuds intermédiaires si un moindre nombre de sauts peut être effectuée. \\
...
\begin{itemize}
    \item Path : Le chemin telle que renvoyé par l'algo
    \item PathExcl : Si l'on veut exclure un noeud précis.
    \item TreeExcl : Pour avoir l'arbre complet du chemin avec exclusion\\
\end{itemize}

Les tests sonts simples, on créer plusieurs \gls{mg} auquel on applique chacunes des macros,
on utilise le manager le plus simple possible pour ne rendre le test trop lourd, on applique en dernier la méthode Hop et la méthode SABR 
...
sont réunis dans un fichiers à part (test\_helpers.rs). De cette manière, seul les tests sont présents dans leur fichiers cibles. \\
...
5 noeuds qui n'ont pas de manager pour faire au plus simple et de 5 contacts.

Exemple le premier contact émet du noeud 0 au noeud 1, début du contact à 0 et fin à 10, débit de 1 octets/seconde et aucun délai de propagation.
